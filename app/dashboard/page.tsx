      ...
      <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
        ...
        <div className="flex gap-2">
          ...
          >
            ...
            Economia Estimada: R$ {analyzedData ? analyzedData.analysis.financial?.total_value ? (analyzedData.analysis.financial.total_value * 0.2).toFixed(2) : "45,00" : "45,00"}
          </Badge>
        </div>
      </div>
                ...
                <CardContent>
                  {isAnalyzing ? (
                    ...
                    <div className="space-y-6 animate-in fade-in duration-500">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="bg-secondary/30 border-border hover:bg-secondary/50 transition-colors rounded-xl border p-4">
                          <div className="text-muted-foreground mb-1 text-xs font-medium uppercase tracking-wider">Valor Total</div>
                          <div className="text-primary text-2xl font-bold">
                            R$ {analyzedData.analysis.financial?.total_value?.toFixed(2) ?? "0.00"}
                          </div>
                          <div className="text-muted-foreground mt-1 text-xs">
                            Vencimento: {analyzedData.analysis.financial?.due_date ?? "Não identificado"}
                          </div>
                        </div>
                        <div className="bg-secondary/30 border-border hover:bg-secondary/50 transition-colors rounded-xl border p-4">
                          <div className="text-muted-foreground mb-1 text-xs font-medium uppercase tracking-wider">Consumo</div>
                          <div className="flex items-baseline gap-1">
                            <div className="text-foreground text-2xl font-bold">
                              {analyzedData.analysis.consumption?.total_m3 ?? analyzedData.analysis.consumption?.total_kwh ?? "0"}
                            </div>
                            <span className="text-muted-foreground text-sm font-medium">
                              {analyzedData.analysis.consumption?.total_m3 ? "m³" : "kWh"}
                            </span>
                          </div>
                          <div className="mt-1 flex items-center gap-1 text-xs">
                            <Badge variant={analyzedData.analysis.consumption?.status === "HIGH" ? "destructive" : "secondary"} className="h-5 px-1.5 text-[10px]">
                              {analyzedData.analysis.consumption?.status ?? "N/A"}
                            </Badge>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="grid gap-2">
                            {analyzedData.analysis.action_items?.slice(0, 3).map((item: any, i: number) => (
                              <div key={i} className="group bg-card hover:shadow-sm border-border hover:border-primary/30 relative overflow-hidden rounded-lg border p-2.5 transition-all">
                                <div className={`absolute left-0 top-0 bottom-0 w-1 ${item.priority === 'HIGH' ? 'bg-red-500' : item.priority === 'MEDIUM' ? 'bg-yellow-500' : 'bg-blue-500'}`} />
                                <div className="flex items-start justify-between gap-2 pl-2">
                                  <div>
                                    <p className="text-foreground text-xs font-medium group-hover:text-primary transition-colors line-clamp-1">
                                      {item.action}
                                    </p>
                                  ...
                                  <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 dark:bg-green-900/20 dark:text-green-400 dark:border-green-900/50 whitespace-nowrap text-[10px] h-5 px-1">
                        ...
                        <Dialog>
                          <DialogTrigger asChild>
                            <Button
                              variant="outline"
                              size="sm"
                              className="border-border text-muted-foreground hover:text-foreground flex items-center gap-2 bg-transparent text-xs h-8"
                            >
